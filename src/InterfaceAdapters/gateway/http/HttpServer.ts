...
export class HttpServer {
	...
	registerController(controller: IHttpController) {
		controller.endpoints.forEach(endpoint => {
			...
			const keys: Key[] = []; // Store the keys for the url, example '/api/user/:id'
			this.endpoints.push({
	...
	async onRequest(req: HttpRequest): Promise<HttpResponse>  {
		...
		const endpoints = this.endpoints.filter((end) => end.url.regex.exec(url) !== null);
	...
	private parseUrlParams(url: string, endpoint: ServerEndpoint): { [key: string]: string } {
		...
		if(!urlParams) return {}
		return Object.fromEntries(endpoint.url.keys.map( (key, index) => [key.name, urlParams[index + 1]]));
	}
}
