...
import {
	...
	FindOneForm, Operation, PaginateForm, Relations,
	RepositoryModel,
	Where
...
export default class TypeOrmModelAdapter<Model extends ObjectLiteral> implements RepositoryModel<Model> {
	...
	async paginate(query: PaginateForm<Model>): Promise<Result<{ items: Model[]; total: number }, DatabaseError>> {
		let where: FindOptionsWhere<Model>[] | FindOptionsWhere<Model> = {}
		if(query?.where) where = Array.isArray(query.where) ? query.where.map(item => whereConverter(item)) : whereConverter(query.where);
		try {
			const paginate = await this.model.findAndCount({
				where,
				select: query.select,
				order: query.order as FindOptionsOrder<Model>,
				relations: query.relations ? convertRelations<Model, Relations<Model>>(query.relations) : undefined,
				skip: (query.page - 1) * query.pageSize,
				take: query.pageSize,
			});
			return Ok({items: paginate[0], total: paginate[1], page: query.page, limit: query.pageSize});
		} catch (e) {
			return Err(new DatabaseError(String(e)));
		}
	}

	async update(model: DeepPartial<Model>): Promise<Result<Model, DatabaseError>> {
		try {
			await this.model.update(model.id, model);
			const updateRes = await this.model.findOne({ where: { id: model.id }});
			if (!updateRes) return Err(new DatabaseError())
			return Ok(updateRes);
		} catch (e) {
			return Err(new DatabaseError(String(e)));
		}
